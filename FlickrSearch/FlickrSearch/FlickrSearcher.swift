...
let apiKey = "d52f65b6a7c1376da515bfe3d1a4d7a8"
...
class FlickrPhoto : Equatable {
  ...
  func flickrImageURL(size:String = "m") -> NSURL {
    print("http://farm\(farm).staticflickr.com/\(server)/\(photoID)_\(secret)_\(size).jpg")
...
class Flickr {
    ...
    func searchFlickrForTerm(searchTerm: String, completion : (results: FlickrSearchResults?, error : NSError?) -> Void) {
        ...
        NSURLConnection.sendAsynchronousRequest(searchRequest, queue: processingQueue) { (response, data, error) -> Void in
            if error != nil {
    ...
    private func flickrSearchURLForSearchTerm(searchTerm:String) -> NSURL {
        ...
        let escapedTerm = searchTerm.stringByAddingPercentEncodingWithAllowedCharacters(NSCharacterSet.whitespaceAndNewlineCharacterSet())!
